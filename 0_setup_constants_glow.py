...
# add widgets to control n_samples and n_variants

# COMMAND ----------

#genotype matrix
n_samples = 10000 # 500000 # 10K, 35K, 70K, 150K, 500K, 1M
n_variants = 500000 # 1000000 # 100K, 250K, 500K, 1M
...
dbfs_file_path = dbfs_home_path / "genomics/photon/data/pandas/"
dbutils.fs.mkdirs(str(dbfs_file_path))

dbfs_file_fuse_path = dbfs_fuse_home_path / "genomics/photon/data/pandas/"
...
vcfs_path = str(dbfs_home_path / "genomics/photon/data/1kg-vcfs-autosomes")
vcfs_path_local = str(dbfs_fuse_home_path / "genomics/photon/data/1kg-vcfs-autosomes")

os.environ['vcfs_path_local'] = vcfs_path_local

simulate_prefix = str(dbfs_home_path / f"genomics/photon/data/delta/simulate_{n_samples}_samples_{n_variants}")
simulate_prefix_local = str(dbfs_fuse_home_path / f"genomics/photon/data/delta/simulate_{n_samples}_samples_{n_variants}") 

output_vcf_delta = str(dbfs_home_path / f'genomics/photon/data/delta/1kg_variants_pvcf.delta')
...
output_delta_transformed = simulate_prefix + "_variants_pvcf_transformed.delta"
output_hwe_path = str(dbfs_home_path / f"genomics/photon/data/results")
output_hwe_plot = str(dbfs_fuse_home_path / f"genomics/photon/data/results/simulate_{n_samples}_samples_{n_variants}_hwe.png")
...
delta_path = str(dbfs_home_path / 'genomics/photon/data/delta/simulate_'.format(user))
...
pandas_path = str(dbfs_fuse_home_path / ('genomics/photon/data/pandas/simulate_'.format(user) + str(n_samples) + '_samples_'))
...
variant_db_name = cleaned_username + "_photon_variant_db"
variant_db_quarantine_table = cleaned_username + "_photon_variant_db.quarantine"
...
dircache_file_path = str(dbfs_home_path / ("genomics/photon/reference"))
dbutils.fs.mkdirs(dircache_file_path)
dircache_file_path_local = str(dbfs_fuse_home_path / "genomics/photon/reference")
os.environ['dircache_file_path_local'] = dircache_file_path_local

log_path = str(dbfs_fuse_home_path / 'genomics/photon/data/logs/')
...
output_exploded_delta = str(dbfs_home_path / f'genomics/photon/data/delta/explode_{n_samples}_samples_{n_variants}_variants_pvcf.delta')
gff_annotations = str(dbfs_home_path / f'genomics/photon/data/delta/gff_annotations.delta')
...
gene_overlap_variants_delta = str(dbfs_home_path / f'genomics/photon/data/delta/gene_overlap_variants.delta')
