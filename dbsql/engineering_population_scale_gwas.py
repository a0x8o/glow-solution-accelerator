...
# MAGIC 
# MAGIC ### Engineering population scale GWAS with
# MAGIC 
# MAGIC ## <img width="90px" src="https://spark.apache.org/docs/latest/img/spark-logo-hd.png"> __+__ <img width="170px" src="https://docs.delta.io/latest/_static/delta-lake-logo.png"> __+__  <img width="100px" src="https://mlflow.org/docs/0.7.0/_static/MLflow-logo-final-black.png">
...
# MAGIC 
# MAGIC To that end we have developed an extensible Spark-native implementation of GWAS
# MAGIC 
# MAGIC That leverages the high performance big-data store, Delta Lake
# MAGIC 
# MAGIC And uses mlflow to log parameters, metrics and plots associated with each run
...
dbutils.fs.mkdirs("dbfs:/mnt/gwas_test")
...
phenotype_path = "dbfs:/databricks-datasets/genomics/1000G/phenotypes.normalized"
sample_info_path = "dbfs:/databricks-datasets/genomics/1000G/samples/populations_1000_genomes_samples.csv"

delta_silver_path = "dbfs:/mnt/gwas_test/snps.delta"
delta_gold_path = "dbfs:/mnt/gwas_test/snps.qced.delta"
principal_components_path = "dbfs:/mnt/gwas_test/pcs.delta"
gwas_results_path = "dbfs:/mnt/gwas_test/gwas_results.delta"
...
# MAGIC 
# MAGIC Using Databricks' VCF reader, which enables variant call format (VCF) files to be read as a Spark Datasource directly from cloud storage 
# MAGIC 
...
vcf_view = spark.read.format("com.databricks.vcf"). \
...
# MAGIC 
# MAGIC Which are built into the Databricks HLS Runtime
...
# MAGIC 
# MAGIC Note: `array_to_sparse_vector` is a function built into the HLS Runtime
...
# MAGIC 
# MAGIC Note: `genotype_states` is a function in the HLS Runtime 
# MAGIC 
# MAGIC that converts an genotypes array, e.g. `[0,1]` into an integer containing the number of alternate alleles, e.g. `1`
...
# MAGIC gwas_df <- read.df("s3a://dbgenomics.us-west-2/dbgenomics.data/1kg/1kg.all.09162019.gwas_results.delta", source="delta")
